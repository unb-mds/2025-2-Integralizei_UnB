...
def get_db():
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_disc_aluno ON disciplinas_cursadas(aluno_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_int_aluno ON integralizacoes_semestre(aluno_id)")
    conn.commit()
    return conn
    ...
    if request.method == "POST":
        ...
        if not f or not f.filename.lower().endswith(".pdf"):
            return render_template("index.html",
                                   status="erro",
                                   msg="Envie um PDF em formato .pdf")
        ...
        with open(save_path, "rb") as fh:
            if fh.read(5) != b"%PDF-":
                ...
                return render_template("index.html",
                                       status="erro",
                                       msg="Arquivo não é um PDF válido.")
